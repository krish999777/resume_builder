...
const EducationSchema = z
  .object({
    ...
    startDate: z.coerce
      .date()
      .max(new Date(), { error: "Start date cannot be in the future" }),
    endDate: z.coerce.date().optional(),
  })
  .refine(
    (data) =>
      data.startDate.getTime() <
      (data.endDate ? data.endDate.getTime() : new Date().getTime()),
    { error: "Cannot have start date after end date" },
  );
...
const ResumeSchema = z.object({
  title: z
    .string({ error: "Title must be present" })
    .trim()
    .min(1, {
...
export async function createResumeController(req: Request, res: Response) {
  ...
  try {
    ...
    if (!result.success) {
      return res
        .status(400)
        .json({ error: result.error.issues.map((err) => err.message) });
    }
...
const QuerySchema = z.object({
  ...
  sort: z
    .enum(["skill", "creation"], {
      error: "Sort must be 'skill' or 'creation'",
    })
    .optional(),
});
  ...
  try {
    ...
    if (role === "recruiter") {
      ...
      return res.status(200).json({
        ...
        totalPages:
          resumes.length > 0 ? Math.ceil(resumes[0].totalrows / 10) : 0,
      });
    } else {
...
export async function getEachResumeController(req: Request, res: Response) {
  ...
  try {
    ...
    const {
      user: { email: _email, password: _password, ...cleanUser },
      ...filteredResume
    } = resume;
    return res.status(200).json({
      message: "Resume found",
      data: { ...filteredResume, user: cleanUser },
    });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: "Internal server error" });
...
export async function putResumeController(req: Request, res: Response) {
  //the logic on this controller was too complex and in the process of refactoring the code and making it more DRY, I messed up the typescript so there is any type everywhere.
  const id = req.id;
  ...
  try {
    ...
    function populateArrays(fieldArray: any[], idArray: any[]) {
      ...
      const allArray: { create: any[]; edit: any[]; delete: number[] } = {
        create: [],
        edit: [],
        delete: [],
      };
      for (const ex of fieldArray) {
    ...
    const acResult = populateArrays(
      resume.achievements,
      userResume.achievements,
    );
    if (
      exResult === "Invalid" ||
      prResult === "Invalid" ||
    ...
    return res
      .status(200)
      .json({ message: "Resume edited successfully", data: updatedResume });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: "Internal server error" });
  }
