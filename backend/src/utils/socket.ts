...
export function initSocket(io: Server) {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on(
      "sendMessage",
      async (data: { conversationId: number; message: string }) => {
        ...
        try {
          ...
          await prisma.$transaction(async (tx) => {
            const convo = await tx.conversation.findFirst({
              where: {
                ...
                OR: [
                  { candidateId: socket.data.id },
                  { recruiterId: socket.data.id },
                ],
              },
              select: { id: true },
            });
            if (!convo) {
              throw new Error(
                "Convesation not found or user not in conversation",
              );
            }
          ...
          return socket.emit("error", {
            message: "Error in updating db or emitting message",
          });
        }
      },
    );
    socket.on(
      ...
      async ({ conversationId }: { conversationId: number }) => {
        const isConversation = socket.rooms.has(
        ...
        socket
          ...
          .emit("typingReciever", { conversationId });
      },
    );
  });
}
